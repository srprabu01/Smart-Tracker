                ...
                <div className="p-4 space-y-4">
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-[#191919] border border-[#373737] rounded p-2 text-white focus:border-blue-500 outline-none transition-colors"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input 
                                ...
                                placeholder="e.g. 3x12 or 30 mins"
                                className="w-full bg-[#191919] border border-[#373737] rounded p-2 text-white focus:border-blue-500 outline-none transition-colors"
                            />
                        </div>
                         <div className="flex items-end pb-2">
                             <label className="flex items-center gap-2 cursor-pointer group select-none">
                                <div className={`w-5 h-5 border rounded flex items-center justify-center transition-colors ${isHome ? 'bg-green-600 border-green-600' : 'border-gray-500 group-hover:border-white'}`}>
                                    {isHome && <span className="text-white text-xs">✓</span>}
                                </div>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-[#191919] border border-[#373737] rounded p-2 text-white focus:border-blue-500 outline-none text-sm transition-colors"
                        />
                        <p className="text-[10px] text-gray-500 mt-1">Paste a YouTube link to watch in-app.</p>
                    </div>
                </div>

                <div className="p-4 border-t border-[#373737] flex justify-end gap-2 bg-[#252525]">
                    <button onClick={onClose} className="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors">Cancel</button>
                    <button onClick={handleSave} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-500 text-white rounded font-medium transition-colors">Save Changes</button>
                </div>
            </div>
        </div>
    );
...
const FitnessColumn: React.FC<FitnessColumnProps> = ({ 
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (taskId) {
      onDropTask(taskId, category);
    }
  };

  return (
    <div 
      className={`flex flex-col min-w-[320px] w-full max-w-[400px] rounded-md transition-colors ${isOver ? 'bg-[#252525]' : ''}`}
    ...
    >
      <div className="flex items-center gap-2 mb-3 px-1 pt-2 border-b border-[#373737] pb-2">
        ...
        <div className="ml-auto flex gap-1">
            <button 
                onClick={onAddTask}
                className="text-gray-500 hover:text-gray-300 p-1 rounded hover:bg-[#2c2c2c] transition-colors"
                title="Add task"
            >
      ...
      <div className="flex flex-col gap-2 pb-4 h-full min-h-[200px]">
        {tasks.map(task => {
           ...
           const safeVideoUrl = task.videoUrl || `https://www.youtube.com/results?search_query=${encodeURIComponent(task.title + " exercise tutorial")}`;
           ...
           return (
            ...
            >
                <button 
                   ...
                   className={`mt-1 flex-shrink-0 transition-all transform active:scale-95 ${isDone ? 'text-green-500' : 'text-gray-500 hover:text-gray-300'}`}
                >
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onEditTask(task)}>
                    <div className={`text-sm font-medium break-words leading-tight mb-1.5 ${isDone ? 'text-gray-400 line-through' : 'text-notion-text'}`}>
                        {task.title}
                    </div>
                    
                    <div className="flex items-center gap-2">
                        {task.reps ? (
                            <div className="bg-[#333] text-gray-300 text-[10px] px-1.5 py-0.5 rounded font-mono border border-[#444]">
                                {task.reps}
                            </div>
                        ) : (
                            <div className="text-[10px] text-gray-600 italic">Set reps</div>
                        )}

                        {task.isHomeWorkout && (
                            <div className="text-green-400 bg-green-900/30 px-1 rounded border border-green-900/50 flex items-center justify-center" title="Home Workout">
                ...
                <div className="flex items-center gap-1 self-center">
                    <button 
                        onClick={(e) => {
                            e.stopPropagation();
                            if (embedId) {
                                onPlayVideo(task.videoUrl!, task.title);
                            } else {
                        ...
                        title={embedId ? "Play Video" : "Search on YouTube"}
                    >
                        {embedId ? <IconPlay className="w-4 h-4" /> : <IconExternalLink className="w-4 h-4" />}
                    </button>
                    ...
                    <button 
                        ...
                        className="text-gray-600 hover:text-red-400 transition-colors p-1.5 hover:bg-gray-800 rounded opacity-0 group-hover:opacity-100"
                        title="Delete task"
                    >
        ...
        <button 
          onClick={onAddTask}
          className="flex items-center gap-2 text-gray-500 hover:bg-[#2c2c2c] p-2 rounded text-sm transition-colors mt-1 opacity-50 hover:opacity-100"
        >
          <IconPlus className="w-4 h-4" />
          Add workout
        </button>
...
const FitnessBoard: React.FC<FitnessBoardProps> = ({ tasks, onUpdateTask, onAddTask, onDeleteTask }) => {
  ...
  // Explicitly list the fitness categories to display (exclude GROCERY)
  const categories = [FitnessCategory.ABS, FitnessCategory.GLUTES, FitnessCategory.SNOWBOARD];
  ...
  return (
    <div className="h-full flex flex-col max-w-5xl mx-auto">
        <div className="mb-8 bg-[#202020] rounded-xl p-6 border border-[#333] shadow-sm">
            <div className="flex justify-between items-end mb-2">
                <div>
                    <h2 className="text-xl font-bold text-white">Daily Workout Progress</h2>
                    <p className="text-sm text-gray-400 mt-1">Consistency build peaks! Track your grind.</p>
                </div>
                <div className="text-right">
                    <span className="text-3xl font-bold text-white">{completedTasks}</span>
                    <span className="text-gray-500 text-lg"> / {totalTasks}</span>
                </div>
            </div>
