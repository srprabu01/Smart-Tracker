    ...
    <div className="h-full flex flex-col max-w-5xl mx-auto">
        <div className="mb-8 bg-[#202020] rounded-xl p-6 border border-[#333]">
            <div className="flex justify-between items-end mb-2">
                <div><h2 className="text-xl font-bold text-white">Daily Workout Progress</h2></div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start pb-10">
            {categories.map(cat => (
                <FitnessColumn key={cat} title={cat} category={cat} tasks={tasks.filter(t => t.category === cat)} onAddTask={() => onAddTask(cat)} onDropTask={handleDropTask} onDeleteTask={onDeleteTask} onEditTask={setEditingTask} onPlayVideo={(url, title) => setPlayingVideo({ url, title })} onToggleStatus={handleToggleStatus} />
            ))}
        </div>
