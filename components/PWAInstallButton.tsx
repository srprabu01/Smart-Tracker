...
export const PWAInstallButton: React.FC = () => {
  ...
  if (isInstallable) {
    return (
      <button
        onClick={install}
        className="flex items-center gap-2 rounded-xl bg-app-ink text-white px-3 sm:px-4 py-2 text-xs font-bold uppercase tracking-wider hover:bg-black transition-colors shadow-sm"
      >
    ...
    return (
      <>
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-app-purple-50 text-app-purple-700 border border-app-purple-200 px-3 sm:px-4 py-2 text-xs font-bold uppercase tracking-wider hover:bg-app-purple-100 transition-colors shadow-sm"
        >
          <Download className="w-4 h-4" />
          ...
          <AnimatePresence>
            {showIOSGuide && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4 sm:p-0"
              >
                <motion.div 
                  initial={{ y: '100%' }}
                  animate={{ y: 0 }}
                  ...
                  className="w-full max-w-sm rounded-[2rem] bg-white p-8 shadow-2xl relative mb-12 sm:mb-0"
                >
                  <button 
